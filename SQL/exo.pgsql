...
-- Exercice 1 ¶
-- Obtenez le nombre total d'utilisateurs créés par an.

-- Utilisez la table des utilisateurs pour cet exercice.

-- La sortie doit contenir l'année à 4 chiffres et le nombre.

-- Utilisez des fonctions spécifiques à la date pour obtenir l'année en utilisant created_ts.

-- Assurez-vous de définir des alias pour les colonnes comme created_year et user_count respectivement.

-- Les données doivent être triées par ordre croissant par created_year .

-- Lorsque vous exécutez la requête à l'aide de l'environnement Jupyter, elle peut contenir des décimales pour les entiers. Vous pouvez donc afficher les résultats même avec des décimales.

-- Voici l'exemple de sortie.

SELECT 
    TO_CHAR(created_ts, 'YYYY') AS "année_de_création", 
    COUNT(DISTINCT user_id) AS "user_count"
FROM 
    users
GROUP BY 
    TO_CHAR(created_ts, 'YYYY')
ORDER BY 
    "année_de_création" ASC;





SELECT user_id, user_dob, user_email_id, TO_CHAR(user_dob, 'Day')
FROM users
